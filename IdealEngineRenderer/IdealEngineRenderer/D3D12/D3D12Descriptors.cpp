...
CD3D12DescriptorHandle::CD3D12DescriptorHandle()
	: CPUHandle()
	, GPUHandle()
	, Slot(MAX_uint32)
	, NumDescriptors(0)
...
CD3D12DescriptorHandle::~CD3D12DescriptorHandle() = default;

D3D12_CPU_DESCRIPTOR_HANDLE CD3D12DescriptorHandle::GetCPUDescriptorHandleByIndex(uint32 Index) const
{
	if (IsValid())
	{
...
D3D12_GPU_DESCRIPTOR_HANDLE CD3D12DescriptorHandle::GetGPUDescriptorHandleByIndex(uint32 Index) const
{
	if (IsValid())
	{
		if (bShaderVisible)
...
CD3D12DescriptorHeap::CD3D12DescriptorHeap(std::shared_ptr<CD3D12Device> Device, D3D12_DESCRIPTOR_HEAP_TYPE InHeapType, D3D12_DESCRIPTOR_HEAP_FLAGS InFlags, uint32 InMaxNumDescriptors)
{
	...
	VERIFYD3D12RESULT(Device->GetDevice()->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(DescriptorHeap.GetAddressOf())));

	DescriptorHandleSize = Device->GetDevice()->GetDescriptorHandleIncrementSize(heapDesc.Type);
}

CD3D12DescriptorHandle CD3D12DescriptorHeap::Allocate(uint32 NumDescriptors)
{
	uint32 outSlot = 0;
	...
	CD3D12DescriptorHandle retHandle = CD3D12DescriptorHandle(
...
void CD3D12DescriptorHeap::Free(CD3D12DescriptorHandle& Handle)
{
...
void CD3D12DescriptorHeap::InitAllocator()
{
	// 첫 범위를 크게 하나로 만든다.
	// |<-----------Range----------->|
...
bool CD3D12DescriptorHeap::Allocate(uint32 NumDescriptors, uint32& OutSlot)
{
	if (const uint32 NumRanges = (uint32)Ranges.size(); NumRanges > 0)
	{
		...
		{
			// 현재 범위
			CRHIDescriptorAllocatorRange& CurrentRange = Ranges[Index];
...
void CD3D12DescriptorHeap::Free(uint32 Slot, uint32 NumDescriptors)
{
	// 현재 범위의 첫 위치
