...
//static void TerminateOnOutOfMemory(std::shared_ptr<CD3D12Device> Device, HRESULT D3DResult)
//{
//	if (ComPtr<IDXGIAdapter1> Adapter = Device->GetAdapter())
//	{
//		const auto& MemoryInfo = Adapter->
//	}
//}
...
extern void VerifyD3D12Result(HRESULT D3DResult, const ANSICHAR* Code, const ANSICHAR* FileName, uint32 Line, std::shared_ptr<CD3D12Device> Device)
{
	if (FAILED(D3DResult))
	{
		const std::string& ErrorString = GetD3D12ErrorString(D3DResult, Device->GetDevice().Get());
